...
Graph::Graph(int size)
    : m_size{size}, m_matrix{size, size}, m_isDirectedGraph{false}, m_isWeightedGraph{false} {}
...
auto Graph::loadGraphFromFile(std::string filename) -> void {
    ...
    for (int i{0}; i < m_size; ++i) {
        for (int j{0}; j < m_size; ++j) {
            ...
            if (!m_isDirectedGraph && m_matrix(i, j) != m_matrix(j, i)) {  //  && i != j
...
auto Graph::exportGraphToDot(std::string filename) -> void {
        ...
        for (int j{0}; j < m_size; ++j) {
            if (m_matrix(i, j) > 0) {
                if (m_matrix(i, j) == 1) {
                    ...
                    outfile << i + 1 << dotSeparator << j + 1 << " [label=" << m_matrix(i, j) << "];"
                            << std::endl;
                }
            }
        }
...
auto Graph::exportUndirectedGraphToDot(std::string filename) -> void {
    std::string filePath{filename + ".dot"};
    std::fstream outfile;
    outfile.open(filePath, std::fstream::out);
    if (!outfile.is_open()) {
        ...
        for (int j{0}; j < m_size; ++j) {
            if (m_matrix(i, j) > 0) {
                ...
                if (m_matrix(i, j) == 1) {
                    ...
                    outfile << first << " -- " << second << " [label=" << m_matrix(i, j) << "];" << std::endl;
                }
            }
        }
    }
...
auto Graph::exportDirectedGraphToDot(std::string filename) -> void {
    std::string filePath = {filename + ".dot"};
    std::fstream outfile;
    outfile.open(filePath, std::fstream::out);
    if (!outfile.is_open()) {
...
auto Graph::operator()(int row, int column) -> int& { return m_matrix.operator()(row, column); }
