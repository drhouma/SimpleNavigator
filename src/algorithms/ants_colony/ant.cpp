...
Ant::Ant(size_t vertex) : vertex_(vertex) { visitedVertices_.push_back(vertex_); }
...
auto Ant::CanMove(Graph& graph) -> bool {
    ...
    for (int i = 0; i < graph.size(); i++) {
        if (i != vertex_ && graph(vertex_, i) != 0 && !ContainsIn(i, visitedVertices_)) {
...
auto comp(std::pair<int, double>& p1, std::pair<int, double>& p2) -> bool { return p1.second < p2.second; }

auto Ant::FindPath(Graph& graph, Matrix<double>& pheromons) -> void {
    while (CanMove(graph)) {
        double sum{0};
